...
// Connect to the browser and start to control it. kage launches Chrome before
// connecting, so a control URL is required. Keeping the automatic launcher out
// of this controller is what prevents Rod's embedded leakless helper from being
// linked into the Windows binary.
func (b *Browser) Connect() error {
	if b.client == nil {
		u := b.controlURL
		if u == "" {
			return errors.New("browser control URL is required")
		}
	...
	if b.monitor != "" {
		return errors.New("browser monitor is not supported by kage's controller")
	}
