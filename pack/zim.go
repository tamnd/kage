...
const syntheticIndexPageLimit = 500
...
// HTML pages by their document titles. The list is capped so a large mirror
// does not produce an unusable page with tens of thousands of elements.
func syntheticIndexHTML(htmlPages []string, pageTitles map[string]string, title string) []byte {
	sorted := append([]string(nil), htmlPages...)
	sort.Slice(sorted, func(i, j int) bool {
		it := strings.ToLower(firstNonEmpty(pageTitles[sorted[i]], sorted[i]))
		jt := strings.ToLower(firstNonEmpty(pageTitles[sorted[j]], sorted[j]))
		if it != jt {
			return it < jt
		}
		return sorted[i] < sorted[j]
	...
	b.WriteString(html.EscapeString(title))
	b.WriteString("</h1><ul>\n")
	visible := min(len(sorted), syntheticIndexPageLimit)
	for _, p := range sorted[:visible] {
		if p == "index.html" {
			continue
		}
		...
		b.WriteString(`">`)
		b.WriteString(html.EscapeString(firstNonEmpty(pageTitles[p], p)))
		b.WriteString("</a></li>\n")
	}
	b.WriteString("</ul>")
	if remaining := len(sorted) - visible; remaining > 0 {
		noun := "pages"
		if remaining == 1 {
			noun = "page"
		}
		_, _ = fmt.Fprintf(&b, "<p>And %d more %s.</p>", remaining, noun)
	}
